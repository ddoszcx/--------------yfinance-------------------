...
tickers = ['AAPL','ASML','AMZN','PDD','GOOGL','META','AMGN','REGN','COST','PEP']
...
def m2_calc(test_portfolio_yield, annual_dev_portfolio, ndq_annual_dev, ndq_annual_yield, annual_free_rate):
    m2 = (test_portfolio_yield - annual_free_rate) * (ndq_annual_dev/annual_dev_portfolio) - (ndq_annual_yield-annual_free_rate)
    return m2
...
def jensen_alpha_calc(test_portfolio_yield, beta, ndq_annual_yield, annual_free_rate): 
    jensen_alpha = test_portfolio_yield - (annual_free_rate + beta * (ndq_annual_yield - annual_free_rate))
    return jensen_alpha
...
def calculate_metrics(weights, y_data=yield_2024, y_nasdaq=yield_nasdaq, ndq_std=nasdaq_std, ndq_annual_yield=nasdaq_annual_yield, ndq_annual_dev=nasdaq_annual_dev, rf_annual=avg_annual_rate24, rf_daily=avg_daily_rate24):
    ...
    jensen_alpha = jensen_alpha_calc(retention_annual, beta, ndq_annual_yield, rf_annual)
...
constraint = (
    ...
    {"type": "ineq", "fun": lambda x: targetstd - annual_dev(daily_std(portfolio_yield(x, yield_2024).dropna()))}, #ограничение на риск <= 20%
    {"type": "ineq", "fun": lambda x: np.sum(x * annual_ret_2024) - targetret} #ограничение на доходность >= 5%
)
initial_guess = [0.1] * 10 #начальные веса (равные для 10 акций)
...
print("\nБета равновзвешенного портфеля за 2024 год:", equal_metrics["Beta"], "\n")
...
with pd.ExcelWriter("close prices test.xlsx", engine = "xlsxwriter") as writer:
