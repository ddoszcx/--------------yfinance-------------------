...
OUTPUT_PATH = Path.cwd() / "dividend_aristocrats_backtest.xlsx"
...
def collect_company_info(ticker_symbol):
    ...
    shares = info.get("sharesOutstanding", pd.NA)
...
def add_valuation_multiples(df, year):
    # Требует наличия колонок Price, Shares Outstanding и финансовых показателей
    ...
    else:  # Если расчет для тестового (другого) года
    ...
    df[f"Market Cap ({year})"] = df[price_col] * df["Shares Outstanding"]  # Расчет рыночной капитализации (Цена акции * Количество акций)
...
def export_to_excel(historical_data, company_info, summary_data, test_data):
    metrics = [
        ...
        "Gross Margin", "Operating Margin", "Net Profit Margin", "ROE", "ROIC",
